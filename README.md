...
| `0x30` | 전력량계 | `sensor` | 읽기 전용 | 순간/누적 전력량 센서를 제공합니다. 중복 total 값은 정리 대상입니다. |
| `0x33` | 현관 패널 | `sensor` | 읽기 전용 | 일괄소등, 엘리베이터 호출, 거실 조명 제어가 물리적으로 있으나 제어 패킷 실측 전까지 버튼은 만들지 않습니다. |
| `0x36` | 난방/온도조절 | `climate`, `switch`, `number` | 일부 지원 | 그룹 상태를 zone별 climate로 분해합니다. 난방 on/off와 설정온도를 제공합니다. |
