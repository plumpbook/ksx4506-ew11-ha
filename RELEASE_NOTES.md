...
- EW11에 등록된 모든 RS485 통신 지점은 기존처럼 개별 추적하되, `RS485 Device Vitality` 센서에는 정상 장치를 제외한 `problem_devices`만 표시하도록 간결하게 정리했습니다.
- 진단 센서에 전체·정상·응답 없음·오래됨·이벤트 전용·미확인 개수와 문제 장치 수를 함께 제공해 센서 하나로 전체 상태를 확인할 수 있습니다.
- 조명·콘센트·난방 등 F7 제어 결과가 확인되지 않으면 다음 5분 점검을 기다리지 않고 해당 장치를 즉시 `unresponsive`로 표시합니다.
- 문제 장치별 마지막 실패 시각, 실패 원인과 연속 실패 횟수를 제공하며, 이후 정상 응답이 수신되면 자동으로 복구합니다.

## v0.2.29

- RS485 응답 패킷과 상태 조회 결과를 장치별로 추적하는 `RS485 Device Vitality` 진단 센서를 추가했습니다.
- 요청 패킷은 생존 신호에서 제외하고, 장치별 상태를 `healthy`, `unresponsive`, `stale`, `event_only`, `unknown`으로 구분합니다.
- 5분마다 알려진 장치를 한 번씩 저속 조회해 버스를 과도하게 점유하지 않으면서 양방향 통신 상태를 지속적으로 확인합니다.
- Home Assistant 진단 다운로드에 장치별 최근 응답 시각, 응답 횟수와 마지막 점검 결과를 포함합니다.

## v0.2.28

- 일반 STX 스위치·팬·조명 제어도 전송 성공만으로 완료 처리하지 않고 실제 후속 상태 프레임을 확인하도록 변경했습니다.
- EW11 명령 실패를 실행 중지, 큐 포화, 큐 시간 초과, 연결 불가, 쓰기 시간 초과, 쓰기 실패로 구분해 `EW11 Link` 진단 속성에 기록합니다.
- 상태 확인 실패와 전송 실패를 Home Assistant 오류 및 `Packet Quality`의 최근 제어 실패 정보에서 구분할 수 있도록 보강했습니다.

## v0.2.27
- 동일한 장치 상태 프레임이 반복될 때 엔티티 상태를 중복 기록하지 않도록 변경했습니다.
- 프레임 기반 진단 센서 갱신을 초당 1회로 제한해 Home Assistant 이벤트 루프와 Recorder 부하를 줄였습니다.
- EW11 연결 상태 변화는 제한과 관계없이 즉시 반영되도록 유지했습니다.

## v0.2.26
- EW11 전송 큐에 크기와 전체 명령 기한을 적용하고, 만료·취소된 명령이 나중에 전송되지 않도록 수정했습니다.
- 제어 명령부터 ACK와 실제 상태 확인까지 하나의 트랜잭션으로 직렬화하고, 실패를 Home Assistant에 명확히 전달하도록 보강했습니다.
- 가스밸브 잠금, 연결 수명주기, 유효 프레임 기준 RX 상태, discovery 입력 범위와 설정 롤백을 강화했습니다.
- 기본 diagnostics와 로그에서 host 및 raw/payload 패킷 샘플을 숨기고, 명시적으로 허용한 진단에서만 샘플을 노출합니다.
- Python 3.13, uv 잠금 설치, 타입 검사와 고정 SHA GitHub Actions를 CI에 적용했습니다.

## v0.2.25
- 수신 패킷과 관계없는 KS X 4506 엔티티까지 매번 HA 상태를 다시 쓰던 동작을 수정했습니다.
- 실제 상태가 변경된 장치만 갱신하고, EW11 연결 상태 변화처럼 전체 엔티티에 영향을 주는 경우에는 기존처럼 전체 상태를 갱신합니다.

## v0.2.24
- `Packet Quality` 상태값이 최근 10분의 RX 오류와 실제 제어 명령 포기만 반영하도록 변경했습니다.
- 시작 시 복원된 장치 상태 확인 요청 실패는 `tx.state_request_giveups`로만 누적하고, 사용자 제어 실패처럼 warning 로그나 unhealthy 상태를 만들지 않도록 분리했습니다.

## v0.2.23
- 프로토콜 파서를 STX/F7/패킷 로그 모듈로 분리해 유지보수성과 진단 가능성을 높였습니다.
- Python LSP 개발 설정을 추가하고 통합 컴포넌트와 테스트 코드의 타입 진단을 정리했습니다.
- 사용자에게 노출되는 엔티티 동작은 `v0.2.22`와 동일하게 유지했습니다.
...
## v0.2.20
- 조명 모듈의 채널 수가 두 상태 프레임에서 연속 확인된 경우에만 새 채널로 등록하도록 안정화했습니다.
- 일시적으로 상태 바이트가 하나 더 포함된 `0x81` 프레임 때문에 존재하지 않는 조명이 생성되는 문제를 수정했습니다.
- 안정적으로 확인된 실제 토폴로지에서 제외된 기존 가짜 조명 엔티티와 장치 레지스트리 항목을 자동으로 정리합니다.
...
- 전력/온도 같은 센서 값은 실제 패킷 없이 임의 기본값을 만들지 않습니다.
